...
public class GMM {
	...
	private double[] getFeatureVariance(int[][] trnData) {
		...
		for (int j=0; j<dim; j++) {
			...
			//	System.out.println(trnData[t][j]);//trnData都一样
	...
	private SuffStats compSuffStats(int[][] trnData) {
		...
		int nData = trnData.length;
		for (int t = 0; t < nData; t++) {
			double[] post = getPosterior(trnData[t], pi, mu, sigma);
			for (int i = 0; i < nMix; i++) {
				suffStats.ss0[i] += post[i];
				for (int j = 0; j < dim; j++) {
					double tmp = post[i] * trnData[t][j];
					suffStats.ss1[i][j] += tmp;
					suffStats.ss2[i][j] += tmp * trnData[t][j] ;
				}
			}
		}
		return suffStats;
...
//			System.out.println("Example: java sequential.gmm.GMM 784 2 0 ../DataSet ../gmm/");
//			return;
//		}
		...
		for(int i=0; i<10; i++)
		{
			...
			{
				System.arraycopy(trainingData[i][m], 0, trnData[m-1], 0, 784);
			}
			*/
			for (int m=0; m<trainingData[i][0][0]; m++)
			{
				for (int n=0; n<784; n++)
				{
					trnData[m][n] = trainingData[i][m+1][n];
				  	/*    System.out.print(trainingData[i][m+1][n]);
					    if(n==783)
						System.out.println();//都一样
			      	*/
				}
			}
			...
			if (args.length == 5) {
			String paraFile = args[4] + i +".txt"; // gmm/i.txt
